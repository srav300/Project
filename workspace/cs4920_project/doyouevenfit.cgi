...
$sid = "null";
$sid = cookie('CGISESSID') if cookie('CGISESSID');
$session = new CGI::Session(undef, $sid, {Directory=>'/tmp'});

#initializing session

sub print_everything(){
	my $a = $session->param("SESSusr");
	my $b = $session->param("SESSpsw");
	my $ids = $session->id();			
	print qq(session => $ids);
	print ("user = $a--");
	print ("password = $b--");
}

if (cookie('CGISESSID') eq $session->id()){
	...
	if (check_login($a, $b)){
		...
		if (defined param('logout')) {
			...
			my $cookie = cookie(-name => 'CGISESSID' , -value => '', -expires=> '-1d');
			$session->delete();			
			print redirect(-location => '/cgi-bin/doyouevenfit.cgi/home', -cookie => $cookie);
			print("A");
			print_everything();
		   	$bg_handler = 3;
		}
	...
	if (defined param('register')) {
		...
		if (check_register()) {
			create_account();
			#account created, return to signin
			print page_header();
			print("E");			
			print login_screen();
		} else {
		...
		$bg_handler = 2;
		if (check_login( param('username'), param('password'))) {
			#login ok
	    		my $session = new CGI::Session(udef, undef, {Directory=>"/tmp"});
			my $cookie = cookie(CGISESSID => $session->id);
			$session->param("SESSusr",param('username'));
			$session->param("SESSpsw",param('password'));
			print redirect(-url => '/cgi-bin/doyouevenfit.cgi/main', -cookie=>$cookie);
		} else {
			#login failurese
			print page_header();	
			print("H");
	...
	} elsif (true || defined param('home')) {
		#deafult
		print header(-Refresh => "6");
		print("I");
		my $ids = $session->id();			
			print qq(session => $ids);
		print ("default");	
#		
		$sid = cookie('CGISESSID');
		print ("COOKIE = $sid--");
		my $a = $session->param("SESSusr");
		my $b = $session->param("SESSpsw");		
		print ("user = $a--");
...
sub friend() {
	...
	<pre> </pre>
	<input type="submit" name="search_friends" value="SEARCH" class="button" style="height:45px;width:200px;"><br>
	<pre> </pre> <pre> </pre>);
...
sub login_screen(){
	...
	if (defined param('username') && defined param('password') && !check_login(param('username'),param('password'))) {	# if user has failed to log in notify them	
